...
const sections: HousingSection[] = [
  ...
  {
    icon: Building2,
    title: '微型社宅新構想 — 5 坪上班族專用住宅',
    subtitle: '2026-05 專家提議｜都會區單身青年租屋的突破性解法',
    items: [
      { text: '專家學者提出「微型社宅」新產品構想（extra.rakuya 5/21）：針對都會區單身上班族，推出 5 坪（約 16.5 平方公尺）極簡社宅，月租 $3,000-$4,000。為傳統套房租金（$10,000+）的三分之一，大幅降低租屋負擔', highlight: true },
      { text: '微型社宅核心設計：①一體成型衛浴（日本 UNIT BATH 技術，1 坪）②折疊式家具（床/桌/椅全可收納，白天為起居空間、晚上為臥室）③共用設施（每層樓設有共用廚房、交誼廳、洗衣間，降低室內面積需求）④智慧門鎖+快遞櫃（適合電商世代）', highlight: true },
      { text: '國際案例參考：日本東京「Ququri 微型社宅」（約 3 坪）自 2019 年營運，入住率高達 98%，住戶平均居住 2.3 年後因結婚/轉職自然退租 — 證明微型社宅可作為青年居住階梯的第一步' },
      { text: '台灣試點建議：優先於台北捷運末端站周邊（淡水、土城、迴龍、南港展覽館）選址，釋出閒置公有地。以每戶建造成本 $80 萬計，5,000 戶僅需 $40 億（相當於一座大型社宅的預算），可緩解台北最急迫的青年租屋需求' },
      { text: '此為針對「非家庭導向」的新住宅產品類別 — 傳統社宅默認家庭需求（二房/三房為主），微型社宅專注服務單身青年，填補現行社宅體系的最大產品缺口' },
      { text: '潛在挑戰：①法規鬆綁（現行最小居住單元標準為 8 坪，需修法放寬）②社會接受度（5 坪是否等於「籠居」？需搭配高品質共用設施改變觀感）③管理機制（短期租約管理 + 退租後快速翻新）④選址策略（交通便利性為首要考量，步行 10 分鐘內需達捷運站）' },
    ],
    stats: [
      { label: '微型社宅坪數', value: '5 坪' },
      { label: '預估月租', value: '$3,000-4,000' },
      { label: '日本入住率', value: '98%' },
    ],
  },
  {
    icon: TrendingUp,
